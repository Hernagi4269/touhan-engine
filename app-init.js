(function(){
  ...
  function syncMeta(){const n=Math.max(1,Number($('genRound').value)||1),d=$('genDate').value||today();$('genDayId').value=`study-${d.replaceAll('-','')}-${String(n).padStart(2,'0')}`;$('genTitle').value=d.replace(/-/g,'/')}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    $('genDate').value=today();syncMeta();$('genDate').addEventListener('change',syncMeta);$('genRound').addEventListener('input',syncMeta);$('genMode').addEventListener('change',()=>{const m=$('genMode').value;$('generateDailyBtn').textContent=m==='one_by_one'?'一問一答120問生成':m==='practice60'?'総合演習60問生成':'本番形式120問生成';});
    ...
    $('generateDailyBtn').onclick=async()=>{try{if(!report)await validate();const mode=$('genMode').value;generated=TouhanGenerator.generate({questions:report.valid,date:$('genDate').value,dayId:$('genDayId').value.trim(),title:$('genTitle').value.trim(),mode,kind:$('genKind').value});renderGenerated(generated);setStatus(`${mode==='one_by_one'?'一問一答120問':mode==='practice60'?'総合演習60問':'本番形式120問'}を生成しました。統合JSONを学習アプリへ取り込めます。`,'ok')}catch(e){setStatus(`生成失敗：${e.message}`,'err')}};
