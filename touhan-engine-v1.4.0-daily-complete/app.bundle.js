...
(function(){
  ...
  const HISTORY_KEY='touhan.engine.generator.history.v120',LEARNING_KEY='touhan.engine.learning.state.v1',EXPLANATION_OVERRIDE_KEY='touhan.engine.tkdb.explanation.overrides.v1';
  ...
  function conciseOneByOneExplanation(statement,truth){
    ...
    // 正しい内容を安全に自動復元できない場合は、未登録として監査対象に残す。
  ...
  function toExamQuestion(q,no){
    return {
      ...
      chapter:q.chapter,
      theme:`東京都${q.year}年度`,
      knowledgeId:q.question_id,
      knowledge_id:q.question_id,
      tkdbKnowledgeIds:(SHORT_EXPLANATION_MASTER.__questionMap?.[q.question_id]?.knowledgeIds)||[],
  ...
  let SHORT_EXPLANATION_MASTER={};
  function explanationOverrides(){try{return JSON.parse(localStorage.getItem(EXPLANATION_OVERRIDE_KEY)||'{}')}catch{return {}}}
  function saveExplanationOverrides(records){localStorage.setItem(EXPLANATION_OVERRIDE_KEY,JSON.stringify(records||{}))}
  function setShortExplanationMaster(data){
    const base={...((data&&data.records)||{})},overrides=explanationOverrides();
    for(const [id,row] of Object.entries(overrides)){if(id&&row&&typeof row==='object')base[id]={...(base[id]||{}),...row};}
    SHORT_EXPLANATION_MASTER=base;SHORT_EXPLANATION_MASTER.__questionMap=(data&&data.questionMap)||{};
    return Object.keys(base).filter(k=>k!=='__questionMap').length;
  }
  function mergeExplanationUpdates(payload){
    const rows=Array.isArray(payload?.tkdbExplanationUpdates?.records)?payload.tkdbExplanationUpdates.records:Array.isArray(payload?.explanationUpdates)?payload.explanationUpdates:[];
    if(!rows.length)return 0;
    const overrides=explanationOverrides();let count=0;
    for(const item of rows){
      const sourceId=String(item.knowledgeId||item.knowledge_id||'').trim();
      if(!sourceId)continue;
      const incoming={knowledgeId:sourceId,tkdbKnowledgeId:String(item.tkdbKnowledgeId||'').trim(),detailedExplanation:String(item.detailedExplanation||item.explanation||'').trim(),correctKnowledge:String(item.correctKnowledge||'').trim(),commonTrap:String(item.commonTrap||'').trim(),memoryTip:String(item.memoryTip||'').trim(),relatedKnowledge:String(item.relatedKnowledge||'').trim(),verificationStatus:String(item.verificationStatus||'assistant_generated'),safeToDisplayDetailedExplanation:item.safeToDisplayDetailedExplanation!==false,sourceType:String(item.sourceType||'assistant_generated'),updatedAt:String(item.updatedAt||payload.generatedAt||new Date().toISOString())};
      if(!incoming.detailedExplanation&&!incoming.correctKnowledge&&!incoming.commonTrap&&!incoming.memoryTip&&!incoming.relatedKnowledge)continue;
      overrides[sourceId]={...(overrides[sourceId]||{}),...incoming};count++;
    }
    saveExplanationOverrides(overrides);
    for(const [id,row] of Object.entries(overrides))SHORT_EXPLANATION_MASTER[id]={...(SHORT_EXPLANATION_MASTER[id]||{}),...row};
    return count;
  }
  function explanationRecordFor(id){return SHORT_EXPLANATION_MASTER[String(id||'')]||null}
  function toOneByOneQuestion(q,no){
    const answer=q.truth?'○':'×';
    const record=explanationRecordFor(q.question_id);
    const generatedShort=conciseOneByOneExplanation(q.statement,q.truth);
    const short=record?.shortExplanation||generatedShort;
    const detailed=record?.detailedExplanation||record?.explanation||detailedOneByOneExplanation(q.statement,q.truth,short);
    return {no,chapter:q.chapter,theme:`東京都${q.year}年度`,knowledgeId:q.question_id,knowledge_id:q.question_id,tkdbKnowledgeId:record?.tkdbKnowledgeId||'',source:`過去問（東京都${q.year}年度 問${q.question_no}）`,source_question_id:q.source_question_id||'',topic_keys:topicKeys(q).filter(k=>!k.startsWith('source:')),answer,text:cleanText(q.statement),shortExplanation:short,explanation:detailed,detailedExplanation:detailed,correctKnowledge:record?.correctKnowledge||'',commonTrap:record?.commonTrap||'',memoryTip:record?.memoryTip||'',relatedKnowledge:record?.relatedKnowledge||'',verificationStatus:record?.verificationStatus||'unverified',safeToDisplayDetailedExplanation:!!record?.safeToDisplayDetailedExplanation,explanationQuality:record?.quality||shortExplanationQuality(short,q.truth,q.statement),explanationSource:record?.sourceType||'engine_generated',category:'one_by_one',category_label:'一問一答'};
  }
...
(function(){
  ...
  async function loadBundled(){
    setStatus('問題DBとTKDBを読み込んでいます…');
    const [r,m]=await Promise.all([
      fetch('./data/tokyo_master.json',{cache:'no-store'}),
      fetch('./data/tkdb-runtime.json',{cache:'no-store'})
    ]);
    if(!r.ok)throw new Error(`問題DB読込失敗: ${r.status}`);
    if(!m.ok)throw new Error(`TKDB読込失敗: ${m.status}`);
    rawDb=await r.json();
    const explanationMaster=await m.json();
    const explanationCount=TouhanGenerator.setShortExplanationMaster(explanationMaster);
    setStatus(`DB読込完了：問題${rawDb.questions?.length||0}問／TKDB参照${explanationCount}件`,'ok');
    return rawDb;
  }
  ...
  async function importLearningFile(file){const o=JSON.parse(await file.text());if(o?.type!=='touhan_learning_state'||!Array.isArray(o.questions))throw new Error('学習状況JSONではありません');localStorage.setItem(TouhanGenerator.LEARNING_KEY,JSON.stringify(o));const explanationCount=TouhanGenerator.mergeExplanationUpdates(o);renderLearningStatus();setStatus(`学習状況を読み込みました：${o.questions.length}問${explanationCount?`／詳しい解説 ${explanationCount}件をTKDB参照へ統合`:''}`,'ok')}
